...
A working React + Vite inventory app for WalangBrownout Appliances (CCS112 case study):
...
3. Each exported function (`getItems`, `createItem`, `getBatches`, `getAlerts`,
   ...
   call written — just commented out — with the expected request/response shape
   documented in a comment above it. Match that shape in your backend and no
   component changes.
4. Delete `src/data/mockData.js` once you no longer need it.

`getAlerts()` / `resolveAlert()` are the best candidates to wire up first — in
production, resolving an alert would `PATCH /alerts/:id`, exactly as stubbed.
